...
class BookModel:
    ...
    def create_book(self, titulo: str, autor: str, ano: str, preco: float) -> str:
        try:
            result = self.collection.insert_one({"name": titulo, "autor": autor, "ano":ano, "preco":preco})
            book_id = str(result.inserted_id)
            print(f"the Book {titulo} created with id: {book_id}")
            return book_id
    ...
    def update_book(book, book_id: str, titulo: str, autor: str, ano: str, preco: float) -> int:
        try:
            result = self.collection.update_one({"_id": ObjectId(book_id)}, {"$set": {"titulo": titulo, "autor":autor, "ano":ano, "preco":preco}})
            if result.modified_count:
                print(f"Book {book_id} updated with title {titulo}, author {autor}, year {ano}, price {preco}")
            else:
